...
class CommandInterpreter {
    ...
    Table tableDefinition() {
        ...
        if (_input.nextIf("(")) {
            ...
            ArrayList<String> listcolTitles = new ArrayList<String>();
            listcolTitles.add(identifier);
            if (_input.nextIs(",")) {
                while (!_input.nextIs(")")) {
                    _input.next(",");
                    String identifiertow = columnName();
                    listcolTitles.add(identifiertow);
                }
            }
            _input.next(")");
            table = new Table(listcolTitles);
        } else {
            _input.next("as");
            table = selectClause();
        }
    ...
    Table selectClause() {
        ...
        ArrayList<String> namesofCols = new ArrayList<String>();
        namesofCols.add(colname);
        if (_input.nextIs(",")) {
            while (!(_input.nextIs("from"))) {
                if (_input.nextIs(",")) {
                    _input.next(",");
                    namesofCols.add(columnName());
                }
            }
        }
        _input.next("from");
        Table[] tablelist1 = new Table[1];
        Table firstTable = tableName();
        tablelist1[0] = firstTable;
        if (_input.nextIs(",")) {
            _input.next(",");
            Table secondTable = tableName();
            Table[] twotables = new Table[2];
            twotables[0] = firstTable;
            twotables[1] = secondTable;
            if (_input.nextIs("where")) {
                _input.next("where");
                ArrayList<Condition> conds = conditionClause(twotables);
                return firstTable.select(secondTable, namesofCols, conds);
            } else {
                return firstTable.select(secondTable, namesofCols, null);
            }
        } else {
            if (_input.nextIs("where")) {
                _input.next("where");
                ArrayList<Condition> conds = conditionClause(tablelist1);
                return firstTable.select(namesofCols, conds);
            } else {
                return firstTable.select(namesofCols, null);
            }
        }
    }
    ...
    ArrayList<Condition> conditionClause(Table... tables) {
        Condition firstcond = condition(tables);
        ArrayList<Condition> allconds = new ArrayList<Condition>();
        allconds.add(firstcond);
        while (!(_input.nextIs(";"))) {
            if (_input.nextIs("and")) {
                _input.next("and");
            }
            Condition secondcond = condition(tables);
            allconds.add(secondcond);
        }
        return allconds;
    }
    ...
    Condition condition(Table... tables) {
        ...
        Column first = new Column(identifierName, tables);
        if (!(_input.nextIs(Tokenizer.LITERAL))) {
            String newidentifier = name();
            Column second = new Column(newidentifier, tables);
            return new Condition(first, relation, second);
        } else {
            return new Condition(first, relation, literal());
        }
    }
